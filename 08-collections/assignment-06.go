...
func main() {
	...
	wordsCountBySize := getRankBySize(words)
	maxSize, maxCount := getMaxByOccurence(wordsCountBySize)
	fmt.Printf("%d letter words occurs the most with %d occurences\n", maxSize, maxCount)
}

func getRankBySize(words []string) map[int]int {
...
func getMaxByOccurence(wordsCountBySize map[int]int) (maxSize, maxCount int) {
